...
echo "sbatch -a 1-10 -t 2-00:00:00 -p cpu20 -c $CPUNUM --mem-per-cpu=32G -o ${logfolder}LMDEmb_${domain}_${querytype}_${pipeline}_${entitystrategy}.log --open-mode=append run_LMDEmb_single.sh $domain $pipeline $querytype $entitystrategy ;"
sbatch -a 1-10 -t 2-00:00:00 -p cpu20 -c $CPUNUM --mem-per-cpu=32G -o ${logfolder}LMDEmb_${domain}_${querytype}_${pipeline}_${entitystrategy}.log --open-mode=append run_LMDEmb_single.sh $domain $pipeline $querytype $entitystrategy ;
