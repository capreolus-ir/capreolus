...
class RepBERTCLEAR_Class(BertPreTrainedModel):
    """
    Adapted from https://github.com/jingtaozhan/RepBERT-Index/
    """
    def __init__(self, config):
        super(RepBERTCLEAR_Class, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids, valid_mask,
        ...
        # First sample is (query, posdoc), second sample is (query, negdoc) - check out ResidualTrainPairSampler.
        ...
        assert pos_scores.shape == residual.shape, "pos_scores is {} and residual is {}".format(pos_scores.shape,
                                                                                                residual.shape)
...
@Encoder.register
class RepBERTCLEAR(Encoder):
    module_name = "repbertclear"
    config_spec = [
        ConfigOption(
            "pretrained",
            "bert-base-uncased",
    ...
    def instantiate_model(self):
        if not hasattr(self, "model"):
            ...
            self.model = torch.nn.DataParallel(RepBERTCLEAR_Class.from_pretrained(self.config["pretrained"], config=config))
