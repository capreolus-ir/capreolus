...
class PytorchANNTrainer(Trainer):
    """
    Pytorch trainer that should be used with the "denserank" task.
    """
    ...
    def train(self, encoder, train_dataset, dev_dataset, output_path, qrels, metric="map", relevance_level=1):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    p
                    for n, p in encoder.model.named_parameters()
                    if "pool_layer" not in n and not any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0.01,
            },
            {
                "params": [
                    p for n, p in encoder.model.named_parameters() if "pool_layer" not in n and any(nd in n for nd in no_decay)
                ],
                "weight_decay": 0.0,
            },
        ]
        ...
        if hasattr(encoder.model.module, "pool_layer") and encoder.config["poolmethod"] != "mean":
            ...
            optimizer_grouped_parameters += [{"params": encoder.model.module.pool_layer.parameters(), "lr": self.config["lr"]}]
        ...
        if self.config["niters"] == 0:
            ...
            for niter in range(self.config["niters"]):
                ...
                if (niter + 1) % validation_frequency == 0:
                    ...
                    if "pool" in encoder.benchmark.config:
                    ...
                    faiss_logger.info(
                        "pytorch train dev metrics: %s", " ".join([f"{metric}={v:0.3f}" for metric, v in sorted(metrics.items())])
                    )
    ...
    def validate(self, encoder, dev_dataset):
        """
        Encodes all the documents in the validation set and writes them into a FAISS index, and runs a search on it.
        """
        encoder.model.eval()
        num_workers = 1 if self.config["multithread"] else 0
        BATCH_SIZE = 64
        dev_dataloader = torch.utils.data.DataLoader(dev_dataset, batch_size=BATCH_SIZE, pin_memory=True, num_workers=num_workers)
