...
def train(config, modules):
    ...
    train_run = {
        ...
        if qid in benchmark.folds[fold]["train_qids"]
    }
    dev_run = {
        qid: docs
        for qid, docs in best_search_run.items()
        if qid in benchmark.folds[fold]["predict"]["dev"]
    }
    ...
    dev_dataset = PredDataset(
        qid_docid_to_rank=dev_run, extractor=reranker["extractor"]
    )
...
def evaluate(config, modules):
    ...
    if os.path.exists(test_output_path):
        ...
        best_search_run_path = evaluator.search_best_run(
            searcher_run_dir, benchmark, metric
        )["path"][fold]
        best_search_run = searcher.load_trec_run(best_search_run_path)

        docids = set(
            docid for querydocs in best_search_run.values() for docid in querydocs
        )
        reranker["extractor"].create(
            qids=best_search_run.keys(),
            docids=docids,
        ...
        test_run = {
            qid: docs
            for qid, docs in best_search_run.items()
            if qid in benchmark.folds[fold]["predict"]["test"]
        }
        test_dataset = PredDataset(
            qid_docid_to_rank=test_run, extractor=reranker["extractor"]
        )

        test_preds = reranker["trainer"].predict(
            reranker, test_dataset, test_output_path
        )

    metrics = evaluator.eval_runs(
        test_preds,
        benchmark.qrels,
        ["ndcg_cut_20", "ndcg_cut_10", "map", "P_20", "P_10"],
    )
    ...
    for fold in benchmark.folds:
        pred_path = (
            _pipeline_path(config, modules, fold=fold) / "pred" / "test" / "best"
        )
...
def _pipeline_path(config, modules, fold=None):
    pipeline_cfg = {
        ...
        if k not in modules and k not in ["expid", "fold"]
    }
    pipeline_path = "_".join(
        ["task-rerank"] + [f"{k}-{v}" for k, v in sorted(pipeline_cfg.items())]
    )
