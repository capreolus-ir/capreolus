...
class CEDRKNRM(Reranker):
    ...
    def get_replacement_candidates(self, d, threshold=0.8):
        ...
        for passage_id in numpassages:
            decoded_bert_input = self.extractor.tokenizer.bert_tokenizer.decode(d["pos_bert_input"][0][passage_id][1:])
            ...
            for i in range(qlen):
                for j in range(doclen):
                    if simmat[passage_id][i][j] > threshold:
