...
def _verify_metric(metrics):
    ...
    expected_metrics = {
        m for m in VALID_METRICS if not m.endswith("_cut") and m != "P"
    } | {
        m + "_" + str(cutoff)
        for cutoff in CUT_POINTS
        for m in VALID_METRICS
        if m.endswith("_cut") or m == "P"
    }
    for metric in metrics:
        if metric not in expected_metrics:
            raise ValueError(
                f"Unexpected evaluation metric: {metric}, should be one of { ' '.join(sorted(expected_metrics))}"
            )
...
def _eval_runs(runs, qrels, metrics, dev_qids):
    ...
    scores = [
        [metrics_dict.get(m, -1) for m in metrics]
        for metrics_dict in evaluator.evaluate(runs).values()
    ]
...
def search_best_run(runfile_dir, benchmark, primary_metric, metrics=None, folds=None):
    ...
    metrics = (
        []
        if not metrics
        else ([metrics] if isinstance(metrics, str) else list(metrics))
    )
    ...
    for s, score_dict in best_scores.items():
        ...
        test_qrels.update(
            {qid: v for qid, v in benchmark.qrels.items() if qid in test_qids}
        )
