...
def test_extract_segment_long_text(tmpdir):
    ...
    pipeline_config = {
        "passagelen": 30, "slicelen": 20, "tfchannel": True
    }
    ...
    s = "O that we now had here but one ten thousand of those men in England That do no work to-day. Whats he that " \
...
def test_extract_segment_short_text(tmpdir):
    ...
    s = "But we in it shall be rememberèd We few, we happy few, we band of brothers. For he to-day that sheds his " \
...
def test_clean_segments(tmpdir):
    ...
    assert extractor.clean_segments(["hello world", "foo bar"], p_len=4) == ["hello world", "foo bar", extractor.pad_tok, extractor.pad_tok]
...
def test_create_visualization_matrix(monkeypatch, tmpdir):
    pipeline_config = {
        "maxqlen": 5,
        "passagelen": 3, "slicelen": 20, "tfchannel": True
    }
    extractor = DeepTileExtractor(tmpdir, tmpdir, pipeline_config)
    extractor.stoi = {
        "<pad>": 0,
    ...
    level = extractor.create_visualization_matrix(
        ...
        ["hello world", "foo bar", "alice bob"], embeddings_matrix
    )
