...
echo "sbatch -a 1-600 -p gpu20 --mem-per-cpu=64G -o ${logfolder}train_gpu_KNRM_${domain}_${pipeline}_${assessed_set}.log --open-mode=append run_KNRM_train_single.sh $domain $pipeline $entitystrategy $assessed_set;"
sbatch  -a 1-600 -p gpu20 --mem-per-cpu=64G -o ${logfolder}train_gpu_KNRM_${domain}_${pipeline}_${assessed_set}.log --open-mode=append  run_KNRM_train_single.sh  $domain $pipeline $entitystrategy $assessed_set;
